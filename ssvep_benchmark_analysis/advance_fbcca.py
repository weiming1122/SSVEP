...
num_iter = 0
mat_processed = np.zeros([Ns, Nb, Nf, N_channel, N_stim])

for s in range(0, Ns):
    for b in range(0, Nb):
        for f in range(0, Nf):
            # Referencing and baseline correction
            mat_processed[s, b, f, :, :] = fp.preprocess(list_subject_data[s][:, :, f, b], vec_ind_el, ind_ref_el, N_start, N_stop)
    ...
    for b in range(0, Nb):
        ...
        for f in range(0, Nf):
            ...
            for n in range(0, N):
                ...
                mat_filter_train[n] = mne.filter.filter_data(mat_data, fs, l_freq = f_low + n * bw, h_freq = f_high, 
                                                             method = 'fir', l_trans_bandwidth = 2, h_trans_bandwidth = 2, 
                                                             phase = 'zero-double', verbose = False)
            ...
            for k in range(0, Nf):
                ...
                for n in range(N):
                    vec_rho_k[n] = fp.apply_ext_fbcca(mat_filter[n, :, 0:249], mat_Y[k, :, 0:249], mat_filter_train[n, :, 0:249])
            ...
            # Apply threshold
            mat_stand = fp.standardize(mat_filter)
...
accuracy_drop = fp.acc(mat_bool_thresh)
