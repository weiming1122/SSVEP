...
for s in range(0, Ns):
    ...
    for b in range(0, Nb):
        for f in range(0, Nf):
            ...
            # Apply CCA
            for k in range(0, Nf):
                vec_rho[k] = fp.apply_cca(mat_stand[:, 0:250], mat_Y[k, :, 0:250])            # accuracy(1s_1): 24.51; 32.81
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 250:500], mat_Y[k, :, 250:500])        # accuracy(1s_2): 32.89; 42.64
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 500:750], mat_Y[k, :, 500:750])        # accuracy(1s_3): 31.61; 40.71
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 750:1000], mat_Y[k, :, 750:1000])      # accuracy(1s_4): 33.06; 42.38
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 1000:1250], mat_Y[k, :, 1000:1250])    # accuracy(1s_5): 29.55; 39.30
                
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 0:500], mat_Y[k, :, 0:500])            # accuracy(2s_1): 52.31; 63.36
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 250:750], mat_Y[k, :, 250:750])        # accuracy(2s_2): 56.14; 65.75
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 500:1000], mat_Y[k, :, 500:1000])      # accuracy(2s_3): 57.89; 65.49
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 750:1250], mat_Y[k, :, 750:1250])      # accuracy(2s_4): 57.11; 65.45
                
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 0:750], mat_Y[k, :, 0:750])            # accuracy(3s_1): 65.85; 74.43
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 250:1000], mat_Y[k, :, 250:1000])      # accuracy(3s_2): 69.04; 75.48
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 500:1250], mat_Y[k, :, 500:1250])      # accuracy(3s_3): 68.29; 74.83
                    
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 0:1000], mat_Y[k, :, 0:1000])          # accuracy(4s_1): 74.08; 80.92
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 250:1250], mat_Y[k, :, 250:1250])      # accuracy(4s_2): 74.62; 80.81
                    
                # vec_rho[k] = fp.apply_cca(mat_stand[:, 0:1250], mat_Y[k, :, 0:1250])          # accuracy(5s_1): 78.71; 84.83
